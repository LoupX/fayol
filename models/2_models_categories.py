...
db.define_table('category_descriptions',
    Field('category_id', 'reference categories', required=True, notnull=True, readable=False, writable=False),
...
"""
try:
    db.executesql('CREATE INDEX idx_category_descriptions_name ON category_descriptions ((upper(name)));', fetch=False)
except :
    pass
"""
db.categories.date_added.represent = lambda date_added, row: date_added.strftime('%d - %m - %Y')
db.categories.date_modified.represent = lambda date_modified, row: date_added.strftime('%d - %m - %Y')
